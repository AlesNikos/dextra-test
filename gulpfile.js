var gulp = require('gulp'),
  ...
  imagemin2 = require('gulp-imagemin'),
...
gulp.task('sass', function () {
  return gulp.src(src.dev.sass + 'styles.scss')
    .pipe(sass().on('error', sass.logError))
    .pipe(gulp.dest(src.prod.css));
});

gulp.task('css-make', ['sass'], function () {
	return gulp.src(src.prod.css + '/styles.css')
		.pipe(autoprefixer({
...
gulp.task('fonts', function (done) {
  return gulp.src(src.dev.fonts)
    .pipe(gulp.dest(src.prod.fonts))
  done();
});
...
gulp.task('watch', function () {
  ...
  gulp.watch(src.dev.sass + '**/*.scss', ['sass', 'css-make']);
	gulp.watch(src.dev.jsdir + '*/*.js', ['js', 'js-libs', 'prog-js']);
});


// Сборка проекта
gulp.task('build', ['pug', 'sass', 'css-make', 'css-min', 'js', 'js-libs', 'prog-js', 'fonts', 'image-min2', 'icons-min']);
